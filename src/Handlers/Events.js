...
module.exports = async function EventsHandler(event, client) {
    try {
        const activateEvents = client.DB.get('events') || [];
        if (!Array.isArray(activateEvents) || !activateEvents.includes(event.id))
            return;

        let groupMetadata;
        try {
            groupMetadata = await client.groupMetadata(event.id);
        } catch {
            return;
        }
        ...
        if (event.action === 'add') {
            ...
            try {
                ...
                // welcomer-gif may not always be available; wrap in try-catch
                ...
                // Fallback – send plain text welcome
                return void (await client.sendMessage(event.id, {
                    text,
                    mentions: event.participants,
                }));
        ...
        console.error('[EventsHandler] Error:', error.message);
    }
};
